...
# ---------------------------------------------------------------------------
# Property 16: SAP_CREDENTIALS_SECRET values take precedence over env vars
# ---------------------------------------------------------------------------
# Feature: abapgit-adt-operations, Property 16: SAP_CREDENTIALS_SECRET values take precedence over env vars

import json
from unittest.mock import patch

from aws_abap_accelerator.config.settings import load_config, SAPConnectionSettings
import aws_abap_accelerator.config.settings as _settings_module

# settings.py imports SecretReader from 'utils.secret_reader' (non-package path).
# We need to patch the SecretReader that settings.py actually uses.
# The safest approach: patch the 'SecretReader' name in the settings module's namespace.
_SETTINGS_SECRET_READER_PATH = f"{_settings_module.SecretReader.__module__}.SecretReader"


# Strategy: printable ASCII text suitable for usernames/passwords (no null bytes)
_credential = st.text(
...
# A valid SAP host (simple hostname format)
_host = st.just("sap.example.com")


@settings(max_examples=100)
@given(
    secret_username=_credential,
    secret_password=_credential,
    env_username=_credential,
    env_password=_credential,
)
def test_sap_credentials_secret_takes_precedence(
    secret_username: str,
    secret_password: str,
    env_username: str,
    env_password: str,
    ...
    **Validates: Requirements 14.4**
    ...
    For any load_config call where SAP_CREDENTIALS_SECRET is set and the secret
    is successfully retrieved, the username and password in the returned
    SAPConnectionSettings must equal the values from the secret, not the values
    from SAP_USERNAME/SAP_PASSWORD env vars.
    """
    # Build the JSON secret that would be returned by AWS Secrets Manager
    secret_json = json.dumps({"username": secret_username, "password": secret_password})

    env_overrides = {
        "SAP_HOST": "sap.example.com",
        "SAP_CLIENT": "100",
        "SAP_USERNAME": env_username,
        "SAP_PASSWORD": env_password,
        "SAP_CREDENTIALS_SECRET": "my/sap/secret",
        # Disable credential manager path
        "USE_CREDENTIAL_MANAGER": "",
    }

    # Clear the SecretReader AWS cache so each test run is independent
    SecretReader._aws_cache.clear()
    _settings_module.SecretReader._aws_cache.clear()

    with patch.dict(os.environ, env_overrides, clear=False):
        # Patch read_aws_secret on the SecretReader class that settings.py uses.
        # settings.py imports SecretReader from 'utils.secret_reader' (non-package path),
        # which is a different class object than aws_abap_accelerator.utils.secret_reader.SecretReader.
        # We patch via the full dotted path: "<module>.SecretReader.read_aws_secret"
        with patch(_SETTINGS_SECRET_READER_PATH + ".read_aws_secret", return_value=secret_json):
            config: SAPConnectionSettings = load_config()

    assert config.username == secret_username, (
        f"Expected username from secret ({secret_username!r}), "
        f"got {config.username!r} (env username was {env_username!r})"
    )
    assert config.password == secret_password, (
        f"Expected password from secret, but got a different value "
        f"(env password was {env_password!r})"
    )
