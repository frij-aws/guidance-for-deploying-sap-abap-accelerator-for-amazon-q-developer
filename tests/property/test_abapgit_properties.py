...
# ---------------------------------------------------------------------------
# Property 17: Workflow prompt returns required phase content
# ---------------------------------------------------------------------------
# Feature: abapgit-adt-operations, Property 17: Workflow prompt returns required phase content
"""
Property-based tests for the abapgit_development_workflow MCP prompt.
...
def _get_workflow_prompt_fn():
    """
    Return the abapgit_development_workflow prompt function.

    The prompt logic is re-implemented inline here, mirroring the
    implementation registered in fastmcp_server.py.  This keeps the test
    self-contained and free of SAP/network/fastmcp dependencies while still
    validating the content contract of the prompt.
    """
    # Re-implement the prompt function inline so the test is self-contained.
    # This mirrors the implementation in fastmcp_server.py exactly.
    def abapgit_development_workflow(phase=None):
        setup_text = """\
...
a. Write or modify ABAP source files on the local filesystem in abapGit
   serialisation format (one file per ABAP object, following the abapGit
   naming conventions).
...
"""
    ...
    return abapgit_development_workflow


# Required keywords that must appear in the setup phase output
_SETUP_REQUIRED_KEYWORDS = [
    # Step a — boilerplate files
    ".abapgit.xml",
    "src/",
    "README.md",
    # Step b — push to remote
    "git push",
    # Step c — create ABAP package
...
# Required keywords that must appear in the cycle phase output
_CYCLE_REQUIRED_KEYWORDS = [
    # Step b — abaplint
    "abaplint",
    # Step d — pull
    "abapgit_pull",
...
# Phase values to test (None means omitted)
_phase_strategy = st.sampled_from(["setup", "cycle", None])


@settings(max_examples=100)
@given(phase=_phase_strategy)
def test_workflow_prompt_returns_required_phase_content(phase) -> None:
    """
    **Validates: Requirements 15.3, 15.4, 15.5, 15.6**

    For each phase value ("setup", "cycle", or omitted), the returned message
    must contain all steps specified for the requested phase(s) and must
    reference the relevant MCP tool names at each applicable step.
    """
    prompt_fn = _get_workflow_prompt_fn()
    result = prompt_fn(phase)

    assert isinstance(result, str), (
        f"Prompt function must return a string, got {type(result)}"
    )
    assert result.strip(), "Prompt function must return non-empty content"

    if phase == "setup" or phase is None:
        for keyword in _SETUP_REQUIRED_KEYWORDS:
            assert keyword in result, (
                f"Setup keyword {keyword!r} missing from prompt output "
                f"(phase={phase!r}). Output:\n{result}"
            )

    if phase == "cycle" or phase is None:
        for keyword in _CYCLE_REQUIRED_KEYWORDS:
            assert keyword in result, (
                f"Cycle keyword {keyword!r} missing from prompt output "
                f"(phase={phase!r}). Output:\n{result}"
            )

    # When phase is omitted, both setup and cycle content must be present
    if phase is None:
        assert "Repository Setup" in result, (
            "Both-phase output must contain 'Repository Setup'"
        )
        assert "Development Cycle" in result, (
            "Both-phase output must contain 'Development Cycle'"
        )

    # When a specific phase is requested, only that phase's content is returned
    if phase == "setup":
        # Cycle-specific tools should NOT appear in setup-only output
        assert "abapgit_pull" not in result, (
            "Setup-only output must not contain cycle tool 'abapgit_pull'"
        )

    if phase == "cycle":
        # Setup-specific tools should NOT appear in cycle-only output
        assert "abapgit_create_repo" not in result, (
            "Cycle-only output must not contain setup tool 'abapgit_create_repo'"
        )
